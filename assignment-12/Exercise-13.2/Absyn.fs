...
let ppProg fPP p : string =
  ...
  let rec ppExpr' i e =
    ...
    | Fst(e1,aOpt) -> "fst(" + (ppExpr' i e1) + ")" + (fPP aOpt)
    | Snd(e1,aOpt) -> "snd(" + (ppExpr' i e1) + ")" + (fPP aOpt)
...
let tailcalls p : program<'a> =
  let rec tc' tPos e =
    ...
    | Fst(e1,aOpt) -> Fst(tc' false e1, aOpt)
    | Snd(e1,aOpt) -> Snd(tc' false e1, aOpt)
...
let rec freevars e : string Set =
  ...
  | Fst (e1, _) -> freevars e1
  | Snd (e1, _) -> freevars e1
