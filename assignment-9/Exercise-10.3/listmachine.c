...
// mark - Gives a block on the heap a different colour
void mark(word *block, int color) {
  block[0] = Paint(block[0], color);
}

// mark phase - Marks blocks on the heap reachable from the stack black.
...
void sweepPhase() {
  ...
  for (word* heapPtr = heap; heapPtr < afterHeap; heapPtr = next) {
    ...
    // If this block is black, paint it white and continue
    ...
    // If this block is white, paint it blue and join it with adjecent blocks
      ...
      if (prev != 0) {
        prev[1] = (word) heapPtr;
        prev = heapPtr;
      } else // First block in the linked list
        prev = first = heapPtr;
    }
  }

  // Provide the freelist variable a reference to the first free block in the linked list
  freelist = first;
}
